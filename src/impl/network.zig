...
pub const Socket = struct {
    ...
    pub fn bind(self: *Self, ep: EndPoint) !void {
        if (builtin.os.tag == .windows) {
            switch (ep.toSocketAddress()) {
                .ipv4 => |sockaddr| try windows.bind(
                    self.internal,
                    @ptrCast(&sockaddr),
                    @sizeOf(@TypeOf(sockaddr)),
                ),
                .ipv6 => |sockaddr| try windows.bind(
                    self.internal,
                    @ptrCast(&sockaddr),
                    @sizeOf(@TypeOf(sockaddr)),
                ),
    ...
    pub fn listen(self: Self) !void {
        if (builtin.os.tag == .windows) {
            try windows.listen(self.internal, 0);
        } else {
...
const windows = struct {
    ...
    const funcs = struct {
        ...
        extern "ws2_32" fn WSASocketW(af: c_int, type: c_int, protocol: c_int, lpProtocolInfo: ?*anyopaque, g: c_uint, dwFlags: c_uint) callconv(.winapi) ws2_32.SOCKET;
        extern "ws2_32" fn WSAStartup(wVersionRequested: c_ushort, lpWSAData: *winsock.WSAData) callconv(.winapi) c_int;
    };
    fn listen(sock: std.posix.socket_t, backlog: c_int) std.posix.SocketError!void {
        const rc = ws2_32.listen(sock, backlog);
        if (rc != 0) {
            std.log.debug("{}", .{ws2_32.WSAGetLastError()});
            return switch (ws2_32.WSAGetLastError()) {
                .EOPNOTSUPP => return "EOPNOTSUPP",
                else => |err| return unexpectedWSAError(err),
            };
        }
    }
    fn bind(sock: std.posix.socket_t, addr: *const std.posix.sockaddr, len: std.posix.socklen_t) std.posix.SocketError!void {
        const rc = ws2_32.bind(sock, addr, @intCast(len));
        if (rc != 0) {
            std.log.debug("{}", .{ws2_32.WSAGetLastError()});
            return switch (ws2_32.WSAGetLastError()) {
                else => |err| return unexpectedWSAError(err),
    ...
    fn WSACleanup() std.posix.SocketError!void {
        const rc = funcs.WSACleanup();
        if (rc != 0) {
            return switch (ws2_32.WSAGetLastError()) {
                else => |err| return unexpectedWSAError(err),
            };
    ...
    fn WSASocketW(af: c_int, type_i: c_int, protocol: c_int, lpProtocolInfo: ?*anyopaque, g: c_uint, dwFlags: c_uint) std.posix.SocketError!ws2_32.SOCKET {
        const sock = funcs.WSASocketW(af, type_i, protocol, lpProtocolInfo, g, dwFlags);
    ...
    fn WSAStartup(majorVersion: u8, minorVersion: u8) !winsock.WSADATA {
        var wsadata: winsock.WSADATA = undefined;
        const rc = winsock.WSAStartup((@as(std.os.windows.WORD, minorVersion) << 8) | majorVersion, &wsadata);
        if (rc != 0) {
            return switch (ws2_32.WSAGetLastError()) {
                else => |err| return unexpectedWSAError(err),
            };
