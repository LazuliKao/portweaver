...
fn applyFrpcForMapping(
    ...
    for (mapping.frpc) |fwd| {
        ...
        if (!node.enabled) {
            std.log.info("[FRPC] Node '{s}' is disabled, skipping", .{fwd.node_name});
            continue;
        }
...
/// Aggregated FRPC client status
pub const FrpcClientStatus = struct {
    status: []const u8,
    last_error: []const u8,
    node_name: []const u8,
};

/// Get the aggregated status of all FRPC clients
...
pub fn getAggregatedStatus(allocator: std.mem.Allocator) !struct {
    ...
    while (it.next()) |entry| {
        ...
        // Get status from the FRPC client
        const status_json = holder.client.getStatus(allocator) catch |err| {
            std.log.warn("[FRPC] Failed to get status for client: {any}", .{err});
            continue;
        };
        ...
        if (std.mem.eql(u8, parsed.status, "error")) {
            ...
            if (parsed.last_error.len > 0) {
                ...
                if (sanitized_error) |message| {
                    defer allocator.free(message);
                    event_log.logEvent(.frpc_error, message, -1);
                }
...
/// Get the status of a specific FRPC client by node name
