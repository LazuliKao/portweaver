...
let startZigBuildWatch (config: Config) =
    ...
    printfn $"   Command: zig build --watch --debounce %d{config.WatchDebounceMs} -Duci=true -Dubus=true -Dfrpc=true -Dfrps=true -Dddns=true -Dwol=true -Dnftables=true -Dupx=false -Dtarget=%s{config.BuildTarget}"
    printfn ""

    let psi = ProcessStartInfo()
    psi.FileName <- "zig"
    psi.Arguments <- $"build --watch --debounce %d{config.WatchDebounceMs} -Duci=true -Dubus=true -Dfrpc=true -Dfrps=true -Dddns=true -Dwol=true -Dnftables=true -Dupx=false -Dtarget=%s{config.BuildTarget}"
